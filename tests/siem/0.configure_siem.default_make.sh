...
CONF_PARAM="--enable-openmp --with-petsc PETSC_INC=$PETSC_INC PETSC_LIB=$PETSC_LIB"
...
rm -rf ./bin ./obj ./setup ./OUTPUT_FILES ./DATA/Par_file ./DATABASES_MPI
...
sed -i "s:^NCHUNKS .*:NCHUNKS = 6:" DATA/Par_file
...
sed -i "s:^TOPOGRAPHY .*:TOPOGRAPHY  = .false.:" DATA/Par_file
sed -i "s:^ELLIPTICITY .*:ELLIPTICITY = .false.:" DATA/Par_file
sed -i "s:^ATTENUATION .*:ATTENUATION = .false.:" DATA/Par_file
...
sed -i "s:USE_SINSQ_STF .*:USE_SINSQ_STF = .true.:" DATA/Par_file
sed -i "s:PRINT_SOURCE_TIME_FUNCTION .*:PRINT_SOURCE_TIME_FUNCTION = .true.:" DATA/Par_file
